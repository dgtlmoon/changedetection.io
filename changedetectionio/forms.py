...
class commonSettingsForm(Form):
    ...
    notification_body = TextAreaField(
        "Notification Body",
        default=default_notification_body,
        validators=[validators.Optional(), ValidateTokensList()],
    )
    notification_format = SelectField(
        "Notification Format",
        choices=valid_notification_formats.keys(),
        default=default_notification_format,
    )
    trigger_check = BooleanField("Send test notification on save")
    fetch_backend = RadioField(
        "Fetch Method",
        choices=content_fetcher.available_fetchers(),
...
class watchForm(commonSettingsForm):
    ...
    minutes_between_check = html5.IntegerField(
        "Maximum time in minutes until recheck",
    ...
    save_and_preview_button = SubmitField(
        "Save & Preview", render_kw={"class": "pure-button pure-button-primary"}
    )
...
class globalSettingsForm(commonSettingsForm):
    ...
    minutes_between_check = html5.IntegerField(
        "Maximum time in minutes until recheck", [validators.NumberRange(min=1)]
    )
