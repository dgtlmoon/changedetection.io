...
def changedetection_app(config=None, datastore_o=None):
    ...
    def login():
        ...
        if not datastore.data["settings"]["application"]["password"] and not os.getenv(
            "SALTED_PASS", False
        ):
    ...
    def do_something_whenever_a_request_comes_in():
        ...
        app.config["LOGIN_DISABLED"] = (
            ...
            and os.getenv("SALTED_PASS", False) == False
        )
    ...
    def rss():
        ...
        for watch in sorted_watches:
            if not watch["viewed"]:
                ...
                diff_link = {
                    "href": "{}{}".format(
                        base_url, url_for("diff_history_page", uuid=watch["uuid"])
                    )
                }
    ...
    def get_current_checksum_include_ignore_text(uuid):
        ...
        if newest_history_key:
            with open(
                datastore.data["watching"][uuid]["history"][newest_history_key], encoding="utf-8"
            ) as file:
    ...
    def edit_page(uuid):
        ...
        if request.method == "POST" and form.validate():
            ...
            ] = form.subtractive_selectors.data
            ...
            if (
                ...
                and request.args.get("next") == "diff"
                and not form.save_and_preview_button.data
            ):
            ...
            if form.minutes_between_check.data == None:
                form.minutes_between_check.data = datastore.data["settings"]["requests"][
    ...
    def settings_page():
        ...
        if request.method == "GET":
            ...
            form.global_subtractive_selectors.data = datastore.data["settings"]["application"][
                "global_subtractive_selectors"
            ]
            form.global_ignore_text.data = datastore.data["settings"]["application"][
                "global_ignore_text"
            ]
            form.ignore_whitespace.data = datastore.data["settings"]["application"][
                "ignore_whitespace"
            ]
            form.extract_title_as_title.data = datastore.data["settings"]["application"][
            ...
            if (
                not os.getenv("SALTED_PASS", False)
                and request.values.get("removepassword") == "yes"
            ):
        ...
        if request.method == "POST" and form.validate():
            ...
            datastore.data["settings"]["application"][
                "global_subtractive_selectors"
            ] = form.global_subtractive_selectors.data
            datastore.data["settings"]["application"][
                "global_ignore_text"
            ] = form.global_ignore_text.data
            datastore.data["settings"]["application"][
                "ignore_whitespace"
            ] = form.ignore_whitespace.data
            ...
            try:
                with open(filename, "r") as f:
                    ...
                    ignore_rules = (
                        ...
                        + datastore.data["settings"]["application"]["global_ignore_text"]
                    )

                    # .readlines will keep the \n, but we will parse it here again, in the future tidy this up
                    ignored_line_numbers = html_tools.strip_ignore_text(
                        content="".join(tmp), wordlist=ignore_rules, mode="line numbers"
                    )

                    trigger_line_numbers = html_tools.strip_ignore_text(
                        content="".join(tmp), wordlist=watch["trigger_text"], mode="line numbers"
                    )
                ...
                content.append(
                    {
                        "line": "File doesnt exist or unable to read file {}".format(filename),
                        "classes": "",
                    }
                )
        else:
    ...
    def notification_logs():
        ...
        output = render_template(
            ...
            logs=notification_debug_log
            if len(notification_debug_log)
            else ["No errors or warnings detected"],
        )
    ...
    def get_backup():
        ...
        for previous_backup_filename in Path(datastore_o.datastore_path).rglob(
        ...
        with zipfile.ZipFile(
            ...
            # Add the index
            zipObj.write(
                os.path.join(datastore_o.datastore_path, "url-watches.json"),
                arcname="url-watches.json",
            )

            # Add the flask app secret
            zipObj.write(
                os.path.join(datastore_o.datastore_path, "secret.txt"), arcname="secret.txt"
            )
            ...
            with open(os.path.join(datastore_o.datastore_path, list_with_tags_file), "w") as f:
                for uuid in datastore.data["watching"]:
                    url = datastore.data["watching"][uuid]["url"]
        ...
        return send_from_directory(
            os.path.abspath(datastore_o.datastore_path), backupname, as_attachment=True
        )
            ...
            try:
                ...
                if "uuid" in n_object:
                    datastore.update_watch(
                        uuid=n_object["uuid"],
                        update_obj={
                            "last_notification_error": "Notification error detected, please see logs."
                        },
                    )
...
def ticker_thread_check_time_launch_checks():
    ...
    while not app.config.exit.is_set():
        ...
        now = time.time()
        max_system_wide = (
            int(copied_datastore.data["settings"]["requests"]["minutes_between_check"]) * 60
        )
