...
class ChangeDetectionStore:
    ...
    def __init__(
        self, datastore_path="/datastore", include_default_watches=True, version_tag="0.0.0"
    ):
        ...
        try:
            ...
            with open(self.json_store_path) as json_file:
                ...
                for uuid, watch in self.__data["watching"].items():
                    ...
                    self.__data["watching"][uuid][
                        "newest_history_key"
                    ] = self.get_newest_history_key(uuid)
    ...
    def data(self):
        ...
        for uuid, v in self.__data["watching"].items():
            ...
            if not self.__data["watching"][uuid]["fetch_backend"]:
                self.__data["watching"][uuid]["fetch_backend"] = self.__data["settings"][
                    "application"
                ]["fetch_backend"]
    ...
    def scrub_watch(self, uuid, limit_timestamp=False):
        ...
        for timestamp, path in self.data["watching"][uuid]["history"].items():
            if not limit_timestamp or (
                limit_timestamp is not False and int(timestamp) > limit_timestamp
            ):
                self.unlink_history_file(path)
        ...
        for timestamp in del_timestamps:
            ...
            if limit_timestamp:
                ...
                if newest_key:
                    ...
                    try:
                        with open(
                            self.data["watching"][uuid]["history"][str(newest_key)], "rb"
                        ) as fp:
                            content = fp.read()
                        self.data["watching"][uuid]["previous_md5"] = hashlib.md5(
                            content
                        ).hexdigest()
                    except (FileNotFoundError, IOError):
                        self.data["watching"][uuid]["previous_md5"] = ""
