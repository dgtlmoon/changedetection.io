...
def test_check_notification(client, live_server):
    ...
    res = client.post(
        url_for("api_watch_add"), data={"url": test_url, "tag": ""}, follow_redirects=True
    )
    ...
    with open("test-datastore/endpoint-content.txt", "w") as f:
        f.write(
            ";jasdhflkjadshf kjhsdfkjl ahslkjf haslkjd hfaklsj hf\njl;asdhfkasj stuff we will detect\n"
        )
    ...
    with open("test-datastore/notification.txt", "r") as f:
        ...
        assert re.search(
            "Watch UUID: [0-9a-f]{8}(-[0-9a-f]{4}){3}-[0-9a-f]{12}",
        ...
        assert (
            "New ChangeDetection.io Notification - {}".format(test_url) in notification_submission
        )
