"""
Unit tests for the Instant Restock Alert Service.

Tests cover:
- Restock detection logic (is_sold_out: True -> False)
- Non-restock scenarios (availability changes that aren't restocks)
- Instant notification delivery
- High-priority message formatting with 'RESTOCK ALERT' header
- All event details inclusion (name, artist, venue, date, time, price, link)
- Tag webhook routing
- Notification logging with type='restock'
- Error handling and edge cases

Run with: pytest tasks/test_restock_alert.py -v
"""
...
from tasks.restock_alert import (
    RestockAlertResult,
    RestockAlertService,
    check_availability_change_for_restock,
    send_restock_alert,
)
...
def service(mock_store):
    """Create a RestockAlertService with mock store."""
...
# =============================================================================
# RestockAlertResult Tests
# =============================================================================


class TestRestockAlertResult:
    """Tests for RestockAlertResult dataclass."""

    def test_successful_restock_alert(self):
        """Test result for successful restock alert."""
        ...
        assert result.is_restock is True
        assert result.alert_sent is True
        assert result.error_message is None

    def test_restock_detected_but_alert_failed(self):
        """Test result when restock detected but alert failed."""
        result = RestockAlertResult(
            event_id='123',
            is_restock=True,
            alert_sent=False,
    ...
    def test_not_a_restock(self):
        """Test result when change is not a restock."""
        result = RestockAlertResult(
            event_id='123',
            is_restock=False,
            alert_sent=False,
        ...
        assert result.success is False
        assert result.is_restock is False
        assert result.alert_sent is False
    ...
    def test_to_dict(self):
        ...
        assert d['is_restock'] is True
        assert d['alert_sent'] is True
        assert d['notification_result'] is None


# =============================================================================
# Restock Detection Logic Tests
# =============================================================================


class TestRestockDetection:
    """Tests for the restock detection logic."""

    def test_is_restock_true_to_false(self, service):
        """Test that sold_out True -> False is detected as restock."""
        assert service.is_restock(old_is_sold_out=True, new_is_sold_out=False) is True

    def test_is_not_restock_false_to_true(self, service):
        """Test that sold_out False -> True is NOT a restock (it's a sellout)."""
        assert service.is_restock(old_is_sold_out=False, new_is_sold_out=True) is False

    def test_is_not_restock_true_to_true(self, service):
        """Test that sold_out True -> True is NOT a restock (no change)."""
        assert service.is_restock(old_is_sold_out=True, new_is_sold_out=True) is False

    def test_is_not_restock_false_to_false(self, service):
        """Test that sold_out False -> False is NOT a restock (no change)."""
        assert service.is_restock(old_is_sold_out=False, new_is_sold_out=False) is False
...
class TestAlertSending:
    ...
    @patch('tasks.restock_alert.TagNotificationRouter.send_event_notification')
    async def test_sends_alert_on_restock(
        self, mock_send, service, sample_event_uuid, sample_event_data
    ):
        """Test that alert is sent when restock is detected."""
        from tasks.notification_router import NotificationRoutingResult
    ...
    async def test_no_alert_when_not_restock(self, service, sample_event_uuid, sample_event_data):
        """Test that no alert is sent when change is not a restock."""
        result = await service.check_and_alert_restock(
            event_uuid=sample_event_uuid,
            previous_is_sold_out=False,
            current_is_sold_out=True,  # This is a sellout, not restock
            event_data=sample_event_data,
        )
    ...
    @patch('tasks.restock_alert.TagNotificationRouter.send_event_notification')
    async def test_notification_type_is_restock(
        self, mock_send, service, sample_event_uuid, sample_event_data
    ):
        """Test that notification type is 'restock' for logging."""
        from tasks.notification_router import NotificationRoutingResult
        ...
        await service.check_and_alert_restock(
            event_uuid=sample_event_uuid,
            previous_is_sold_out=True,
            current_is_sold_out=False,
            event_data=sample_event_data,
        ...
        # Verify notification_type='restock' was passed
...
class TestMessageContent:
    ...
    @patch('tasks.restock_alert.TagNotificationRouter.send_event_notification')
    async def test_includes_prices(self, mock_send, service, sample_event_uuid, sample_event_data):
        """Test that message includes current prices."""
        from tasks.notification_router import NotificationRoutingResult
    ...
    @patch('tasks.restock_alert.TagNotificationRouter.send_event_notification')
    async def test_includes_high_priority_indicator(
        self, mock_send, service, sample_event_uuid, sample_event_data
    ):
        """Test that message includes high priority indicator."""
        from tasks.notification_router import NotificationRoutingResult
        ...
        call_kwargs = mock_send.call_args.kwargs
        additional_info = call_kwargs['additional_info']
        assert 'Priority' in additional_info
        assert 'HIGH' in additional_info['Priority']

    @patch('tasks.restock_alert.TagNotificationRouter.send_event_notification')
    async def test_availability_is_in_stock(
        self, mock_send, service, sample_event_uuid, sample_event_data
    ):
        """Test that availability is set to 'in_stock' for restocks."""
        from tasks.notification_router import NotificationRoutingResult
        ...
        call_kwargs = mock_send.call_args.kwargs
        assert call_kwargs['availability'] == 'in_stock'
...
class TestEventDataFetching:
    ...
    @patch('tasks.restock_alert.TagNotificationRouter.send_event_notification')
    @patch('tasks.restock_alert.Event.get_by_id')
...
class TestConvenienceFunctions:
    ...
    @patch('tasks.restock_alert.RestockAlertService.check_and_alert_restock')
    async def test_send_restock_alert(
        self, mock_check, mock_store, sample_event_uuid, sample_event_data
    ):
        """Test send_restock_alert convenience function."""
        mock_check.return_value = RestockAlertResult(
            event_id=sample_event_uuid,
            is_restock=True,
            alert_sent=True,
        )
    ...
    @patch('tasks.restock_alert.RestockAlertService.process_availability_change')
    async def test_check_availability_change_for_restock(
        self, mock_process, mock_store, sample_event_uuid, sample_event_data
    ):
        """Test check_availability_change_for_restock convenience function."""
        mock_process.return_value = RestockAlertResult(
            event_id=sample_event_uuid,
            is_restock=True,
            alert_sent=True,
        )
...
class TestProcessAvailabilityChange:
    ...
    @patch('tasks.restock_alert.TagNotificationRouter.send_event_notification')
    async def test_process_availability_change_restock(
        self, mock_send, service, sample_event_uuid, sample_event_data
    ):
        """Test process_availability_change for a restock."""
        from tasks.notification_router import NotificationRoutingResult
    ...
    async def test_process_availability_change_not_restock(
        self, service, sample_event_uuid, sample_event_data
    ):
        """Test process_availability_change for a non-restock."""
        result = await service.process_availability_change(
            event_uuid=sample_event_uuid,
            old_is_sold_out=False,
            new_is_sold_out=True,  # Sellout, not restock
            event_data=sample_event_data,
        )
...
class TestIntegration:
    """Integration-style tests for the full restock alert flow."""

    @patch('tasks.restock_alert.TagNotificationRouter.send_event_notification')
    async def test_full_restock_alert_flow(
        self, mock_send, service, sample_event_uuid, sample_event_data
    ):
        ...
        assert call_kwargs['venue'] == 'United Center'
        assert call_kwargs['availability'] == 'in_stock'
        assert 'HIGH' in call_kwargs['additional_info']['Priority']
